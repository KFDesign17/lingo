          ...
          <div className="min-h-screen p-4 flex flex-col">
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button onClick={() => setDayType('work')} className={`p-4 rounded-2xl border-2 transition-all active:scale-95 ${dayType === 'work' ? 'bg-blue-500/20 border-blue-500 text-blue-400' : 'bg-white/5 border-white/10 text-gray-500'}`}>
                  <Clock size={20} className="mx-auto mb-2" />
                  <span className="text-[10px] font-bold uppercase block">Travail</span>
                </button>
                {Object.entries(absenceLabels).map(([key, config]) => (
                  <button key={key} onClick={() => setDayType(key as any)} className={`p-4 rounded-2xl border-2 transition-all active:scale-95 ${dayType === key ? `${config.color} ${config.textColor}` : 'bg-white/5 border-white/10 text-gray-500'}`}>
            ...
            {dayType === 'work' && (
              <div className="flex-1 space-y-4">
                ...
                <button onClick={addSession} className="w-full py-4 border-2 border-dashed border-white/10 rounded-2xl text-gray-500 active:border-blue-500/50 active:text-blue-400 active:scale-95 transition-all flex items-center justify-center gap-2 font-bold text-sm">
