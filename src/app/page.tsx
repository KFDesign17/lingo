...
export default function LingoDashboard() {
  ...
  const [stats, setStats] = useState({
    ...
    baseSalary: 0,
    totalHoursMonth: 0,
    totalHoursFinancial: 0,
    nightHours: 0,
    nightBonus: 0,
    holidayHours: 0,
    holidayBonus: 0,
    holidayRate: 100,
    socialChargesRate: 21.9,
    fixedDeductions: 0,
    taxRate: 0,
    targetNet: 0,
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      ...
      rate = parsed.hourlyRate || 0;
      hours = parsed.contractHours || 0;
      shifts = parsed.nightShifts || [];
      charges = parsed.socialChargesRate ?? 21.9;
    ...
    if (savedPlanning) {
      try {
        ...
        entries.forEach(entry => {
          ...
          if (isSelectedMonth && !entry.absenceType) {
            entry.sessions?.forEach(session => {
              ...
              totalH += res.h;
              totalHFinancial += res.h;
              totalNightH += res.nH;
              totalBonus += res.b;
              
              if (entry.isHoliday) {
                totalHolidayH += res.h;
                totalHolidayBonus += res.hB;
              }
            });
          }
        });
        ...
        for (let i = historyRange; i >= 0; i--) {
          ...
          tempGraphData.push({
            label: targetDate.toLocaleDateString('fr-FR', { month: 'short' }).replace('.', ''),
            net: mNet,
            isCurrent: i === 0
          });
        }
    ...
    setStats({
      ...
      totalHoursMonth: totalH, 
      totalHoursFinancial: totalHFinancial,
  ...
  function calculateSession(startStr: string, endStr: string, rate: number, shifts: NightShift[], isHoliday: boolean, holidayRate: number) {
    ...
    const hB = isHoliday ? (h * rate * (holidayRate / 100)) : 0;
    return { h, nH, b, hB };
  }

  const toNet = (brut: number) => {
    const charges = brut * (stats.socialChargesRate / 100);
    const netAvantImpot = brut - charges - stats.fixedDeductions;
    return Math.max(0, netAvantImpot * (1 - stats.taxRate / 100));
  };
  ...
  const simBrut = (sim.h * stats.hourlyRate) + sim.b;
  const simNet = toNet(currentTotalBrut + simBrut) - currentTotalNet;
  ...
  const extraProgress = stats.totalHoursMonth > stats.contractHours 
    ? Math.min(((stats.totalHoursMonth - stats.contractHours) / stats.contractHours) * 100, 100) 
    : 0;
    ...
    <div className="flex h-screen bg-[#0a0a0a] text-white font-sans overflow-hidden">
      ...
      `}>
        <div className="p-6 flex items-center justify-between border-b border-white/10">
          <h1 className="text-xl font-bold bg-gradient-to-r from-blue-500 to-cyan-400 bg-clip-text text-transparent italic">
            LINGO PAY
          </h1>
          <button 
            onClick={() => setIsSidebarOpen(false)}
        ...
        <div className="p-4 lg:p-8">
          <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 lg:mb-8 gap-4 text-left">
            ...
            <div className="flex flex-wrap gap-2 w-full lg:w-auto">
              <button 
                ...
                className={`flex-1 lg:flex-initial flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border transition-all font-bold text-[10px] uppercase tracking-wider ${isSimMode ? 'bg-purple-600 border-purple-400 shadow-lg shadow-purple-500/20 text-white' : 'bg-white/5 border-white/10 text-gray-400'}`}
              >
                <Sparkles size={14} /> Simulateur
              </button>
              <button 
                onClick={() => setDisplayDecimal(!displayDecimal)} 
                className="flex-1 lg:flex-initial flex items-center justify-center gap-2 px-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-[10px] font-bold uppercase tracking-wider text-gray-400"
              >
                <RefreshCw size={12} className={displayDecimal ? "text-blue-400" : ""} /> {displayDecimal ? "Comptable" : "Réel"}
              </button>
              <div className="hidden lg:flex w-10 h-10 bg-gradient-to-br from-blue-500 to-blue-700 rounded-full items-center justify-center font-bold uppercase">
                {userName.charAt(0)}
              </div>
            </div>
          </header>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8">
            <StatCard 
              ...
              value={`${(showNet ? baseNet : stats.baseSalary).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} €`} 
              icon={<Wallet className={showNet ? "text-green-400" : "text-blue-400"} />} 
              label={showNet ? `Après ${stats.socialChargesRate}% charges` : "Salaire fixe"} 
              onSwitch={() => setShowNet(!showNet)} 
              isNet={showNet} 
            />
            
            <StatCard 
              title="Primes de Nuit" 
              value={`${(showNet ? toNet((stats.totalHoursFinancial * stats.hourlyRate) + stats.nightBonus) - toNet(stats.totalHoursFinancial * stats.hourlyRate) : stats.nightBonus).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} €`} 
              icon={<Moon className="text-blue-400" />} 
              label={`${formatHours(stats.nightHours)} majorées`} 
            />

            <StatCard 
              title="Bonus Fériés" 
              value={`${(showNet ? toNet(stats.holidayBonus) : stats.holidayBonus).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} €`} 
              icon={<Gift className="text-pink-400" />} 
              label={`${formatHours(stats.holidayHours)} à +${stats.holidayRate}%`} 
            />
            
            <div className="border p-4 lg:p-6 rounded-2xl transition-all bg-[#111] border-white/10 text-left">
              <div className="flex justify-between items-start mb-3 lg:mb-4">
                <div className="p-2 lg:p-3 bg-white/5 rounded-xl border border-white/5 w-fit">
              ...
              <div className="flex bg-white/5 p-1 rounded-xl border border-white/5 gap-1 w-full sm:w-auto">
                {[3, 6, 12].map((range) => (
                  <button 
                    ...
                    className={`flex-1 sm:flex-initial px-3 lg:px-4 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${historyRange === range ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-gray-500'}`}
                  >
                    {range}M
                  </button>
                ))}
              ...
              <div className="absolute inset-x-4 lg:inset-x-8 top-4 lg:top-8 bottom-16 lg:bottom-20 flex flex-col justify-between pointer-events-none">
                {[1, 0.75, 0.5, 0.25, 0].map((ratio) => (
                  <div key={ratio} className="flex items-center gap-2 lg:gap-4 w-full">
                    <span className="text-[9px] text-gray-600 font-bold w-8 lg:w-12 text-right">
                      {(maxNetInGraph * ratio).toLocaleString('fr-FR', { maximumFractionDigits: 0 })}€
                    </span>
                ...
                {graphData.map((month, idx) => {
                  ...
                  return (
                    <div key={idx} className="relative flex-1 flex flex-col items-center group h-full justify-end max-w-[40px] lg:max-w-none">
                      ...
                      <div 
                        className={`w-6 lg:w-10 rounded-t-xl transition-all duration-700 ease-out ${
                          month.isCurrent 
...
function NavItem({ icon, label, active, onClick }: { icon: React.ReactNode, label: string, active: boolean, onClick?: () => void }) {
  return (
    <button 
      onClick={onClick} 
      className={`w-full flex items-center px-4 py-3 gap-3 rounded-xl transition-all ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-gray-400 hover:bg-white/5'}`}
    >
